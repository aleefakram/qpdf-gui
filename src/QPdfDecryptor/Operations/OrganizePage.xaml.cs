...
public partial class OrganizePage : UserControl, IBusyPage
{
    private readonly OrganizeViewModel viewModel = new();
    ...
    public OrganizePage()
    {
        ...
        {
            if (eventArgs.PropertyName is nameof(OrganizeViewModel.IsBusy))
            {
                BusyStateChanged?.Invoke(this, EventArgs.Empty);
                RefreshChrome();
            }
            else if (eventArgs.PropertyName is nameof(OrganizeViewModel.Outcome) or nameof(OrganizeViewModel.StatusPercent))
            {
                RefreshChrome();
            }
        };
    ...
    private void RefreshChrome()
    {
        ...
        if (viewModel.IsBusy)
        {
            Status.ShowRunning($"Reorganizing pages… {viewModel.StatusPercent}%");
        ...
        if (outcome.Succeeded)
        {
            Status.ShowSuccess(outcome.HasWarnings,
                outcome.HasWarnings ? "Organized with warnings" : "Pages organized",
                $"Saved {viewModel.OutputPath}", outcome.Details);
        }
        else
        {
            Status.ShowFailure("Could not organize", outcome.FriendlyError, outcome.Details);
        }
    }
    ...
    private void ChooseOutput_Click(object sender, RoutedEventArgs e)
    {
        ...
        {
            Filter = "PDF files|*.pdf",
            FileName = string.IsNullOrEmpty(stem) ? "organized.pdf" : $"{stem}-organized.pdf",
        };
        if (dialog.ShowDialog(OwnerWindow()) == true)
        {
            viewModel.OutputPath = dialog.FileName;
